loadstring(game:HttpGet"https://raw.githubusercontent.com/Steven55704/idklmaoxd/main/Counter_Blox/Silent_Aim/UILibrary.lua")()



_G['chromeusername'] = "welcome ".. game.Players.LocalPlayer.Name..' (Press = to close this menu)'
...
local chrome = Instance.new("ScreenGui")
chrome.IgnoreGuiInset = true
...
chrome.Name = "chrome"
chrome.Parent = game:GetService("CoreGui")
chrome.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
chrome.DisplayOrder = 2
...
local configfolder = Instance.new("Folder",chrome)
...
colourselection.Parent = chrome --yeetio
...
title.Size = UDim2.new(1, 0, 1, 0)
title.Font = Enum.Font.GothamSemibold
title.Text = "chrome.su | public server | ".. _G['chromeusername'] .." |"
title.TextColor3 = Color3.new(1, 1, 1)
title.TextSize = 10
...
function saverecoveryfile()
	...
	writefile('recovery_chrome.txt',code)
end
function setrecoveryfile()
	local a,b = pcall(function()
		readfile('recovery_chrome.txt')
	end)
	if not string.find(b or "",'not exist') then 
		local code = readfile('recovery_chrome.txt')
...
function checkfile()
	local a,b = pcall(function()
		readfile('config_chrome.txt')
	end)
	if string.find(b or "",'not exist') then 
		writefile('config_chrome.txt','return {}')
	end
end
checkfile()
function savenewconfig()
	...
	bob = bob..'};'
	writefile('config_chrome.txt',bob)
end

function loadsavedfileconfig()
	checkfile()
	local c = readfile('config_chrome.txt')
...
username.Text = _G['chromeusername']
